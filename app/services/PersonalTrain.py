...
def json_to_df(path_or_obj, label=None, username="USER"):
    ...
    for it in data:
        ...
        rows.append({'timestamp_ms': ts if ts is not None else np.nan, 'eye_status': es.get('status', 'OPEN'),
                     ...
                     'pitch': float(hp.get('pitch', 0.0)), 'yaw': float(hp.get('yaw', 0.0)),
...
def train_personal(username, json_focus_path, json_nonfocus_path,
    ...
    train_ds = torch.utils.data.TensorDataset(torch.tensor(X_tr, dtype=torch.float32),
    ...
    with open(os.path.join(CKPT_DIR, f"{username}_header.json"), 'w', encoding='utf-8') as f:
        json.dump(header, f, ensure_ascii=False)

    return {"model": user_model_path, "scaler": user_scaler_path,
            "header": os.path.join(CKPT_DIR, f"{username}_header.json"),
...
if __name__ == "__main__":
    ...
    base_model = os.path.join(CKPT_DIR, "baseline_model.pth")
